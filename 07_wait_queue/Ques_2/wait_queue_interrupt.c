...
static int __init CharDevice_init(void)
{
    ...
    int MAJOR, MINOR;
    dev_t DevNo;
    DevNo = MKDEV(271, 0); // Statically creating a device number
    MAJOR = MAJOR(DevNo);
    MINOR = MINOR(DevNo);
    printk(KERN_ALERT "The Major number is %d and the Minor number is %d.\n", MAJOR, MINOR);
    result = register_chrdev_region(DevNo, 1, "WaitCharDevice1"); // register device region
    if(result < 0)
    {
...
ssize_t NAME_read(struct file *filp, char __user *Ubuf, size_t count, loff_t *f_pos)
{
    char Kbuf[] = "Data writing into user space from kernel space for waking up a process.";
    unsigned long result;
    ssize_t retval;
    result = copy_to_user((char *)Ubuf, (char *)Kbuf, sizeof(Kbuf)); // copy data to user
    cond = 0;
    wake_up(&MyWaitQueue);
    printk("Wake up function implemented.\n");
    if(result == 0)
    {
...
ssize_t NAME_write(struct file *filp, const char __user *Ubuf, size_t count, loff_t *f_pos)
{
    wait_event_interruptible(MyWaitQueue, cond == 0);
    char Kbuf[80];
    unsigned long result;
    ssize_t retval;
