...
static int __init CharDevice_init(void)
{
    ...
    int MAJOR, MINOR;
    dev_t DevNo;
    DevNo = MKDEV(275, 0); // Statically creating a device number
    MAJOR = MAJOR(DevNo);
    MINOR = MINOR(DevNo);
    printk(KERN_ALERT "The Major number is %d and the Minor number is %d.\n", MAJOR, MINOR);
    result = register_chrdev_region(DevNo, 1, "SemphCharDevice1"); // register device region
    if(result < 0)
    {
