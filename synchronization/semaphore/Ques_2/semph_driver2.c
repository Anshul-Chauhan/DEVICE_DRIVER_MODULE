...
static int __init CharDevice_init(void)
{
    ...
    int MAJOR, MINOR;
    dev_t DevNo;
    DevNo = MKDEV(276, 0); // Statically creating a device number
    MAJOR = MAJOR(DevNo);
    MINOR = MINOR(DevNo);
    printk(KERN_ALERT "The Major number is %d and the Minor number is %d.\n", MAJOR, MINOR);
    result = register_chrdev_region(DevNo, 1, "SemphCharDevice2"); // register device region
    if(result < 0)
    {
    ...
    sema_init(&semph1, 1); // Dynamically initialising the semaphore
    sema_init(&semph2, 1);
    return 0;
}
