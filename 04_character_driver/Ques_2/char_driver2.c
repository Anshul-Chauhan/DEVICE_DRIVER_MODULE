...
static int __init CharDevice_init(void)
{
    ...
    int MAJOR, MINOR;
    dev_t DevNo;
    DevNo = MKDEV(257, 0); // Statically creating a device number
    MAJOR = MAJOR(DevNo);
    MINOR = MINOR(DevNo);
    printk(KERN_ALERT "The Major number is %d and the Minor number is %d.\n", MAJOR, MINOR);
    result = register_chrdev_region(DevNo, 1, "MyCharDevice2"); // register device region
    if(result < 0)
    {
...
ssize_t NAME_read(struct file *filp, char __user *Ubuf, size_t count, loff_t *f_pos)
{
    char Kbuf[] = "Data writing into user space from kernel space.";
